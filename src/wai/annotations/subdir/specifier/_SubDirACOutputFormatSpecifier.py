...
class SubDirACOutputFormatSpecifier(SinkStageSpecifier):
    """
    Specifier of the components for writing audio classification instances where
    each audio file is stored in a sub-directory named after the class label.
    """
    @classmethod
    def description(cls) -> str:
        return "Writes audio files to sub-directories named after their class labels."

    @classmethod
    def components(cls) -> Tuple[Type[Component], ...]:
        from ..component import SubDirACWriter
        return SubDirACWriter,
